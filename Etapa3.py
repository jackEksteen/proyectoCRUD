...
DATABASE = 'inventarioModificado.db'
...
# -------------------------------------------------------------------
class Producto:
    def __init__(self, codigo, descripcion, cantidad, precio, pais_origen):
        self.codigo = codigo
    ...
    def modificar(self, nueva_descripcion, nueva_cantidad, nuevo_precio, nuevo_pais):
        self.descripcion = nueva_descripcion
        self.cantidad = nueva_cantidad
        self.precio = nuevo_precio
        self.pais_origen = nuevo_pais
...
class Inventario:
    ...
    def agregar_producto(self, codigo, descripcion, cantidad, precio, pais_origen):
        producto_existente = self.consultar_producto(codigo)
        if producto_existente:
            return jsonify({'message': 'Ya existe un producto con ese código.'}), 400

        #nuevo_producto = Producto(codigo, descripcion, cantidad, precio)
        self.cursor.execute("INSERT INTO productos VALUES (?, ?, ?, ?, ?)", (codigo, descripcion, cantidad, precio, pais_origen))
    ...
    def consultar_producto(self, codigo):
        ...
        if row:
            codigo, descripcion, cantidad, precio, pais_origen = row
            return Producto(codigo, descripcion, cantidad, precio, pais_origen)
        return None

    def modificar_producto(self, codigo, nueva_descripcion, nueva_cantidad, nuevo_precio, nuevo_pais):
        producto = self.consultar_producto(codigo)
        if producto:
            producto.modificar(nueva_descripcion, nueva_cantidad, nuevo_precio, nuevo_pais)
            self.cursor.execute("UPDATE productos SET descripcion = ?, cantidad = ?, precio = ?, pais_origen = ? WHERE codigo = ?",
                                (nueva_descripcion, nueva_cantidad, nuevo_precio, nuevo_pais, codigo))
            self.conexion.commit()
    ...
    def listar_productos(self):
        ...
        for row in rows:
            codigo, descripcion, cantidad, precio, pais_origen = row
            producto = {'codigo': codigo, 'descripcion': descripcion, 'cantidad': cantidad, 'precio': precio, 'pais_origen' : pais_origen}
...
class Carrito:
    ...
    def agregar(self, codigo, cantidad, inventario):
        ...
        nuevo_item = Producto(codigo, producto.descripcion, cantidad, producto.precio, producto.pais_origen)
    ...
    def mostrar(self):
        ...
        for item in self.items:
            producto = {'codigo': item.codigo, 'descripcion': item.descripcion, 'cantidad': item.cantidad,
                        'precio': item.precio, 'pais_origen': item.pais_origen}
...
def agregar_producto():
    ...
    return inventario.agregar_producto(codigo, descripcion, cantidad, precio, pais_origen)
...
def modificar_producto(codigo):
    ...
    nuevo_pais = request.json.get('pais_origen')
    return inventario.modificar_producto(codigo, nueva_descripcion, nueva_cantidad, nuevo_precio, nuevo_pais)
